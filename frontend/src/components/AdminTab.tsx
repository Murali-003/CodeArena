...
import { Terminal, Plus, Edit2, Trash2, ShieldAlert, CheckCircle, Database } from "lucide-react";
...
export default function AdminTab() {
  ...
  const [activeModalTab, setActiveModalTab] = useState<'details' | 'testcases'>('details');
  ...
  const handleAddTestCase = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newTc = await api.post(`/api/problems/${editingProblem.id}/test-cases`, payload);
  ...
  if (loading && problems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-xs font-mono text-zinc-500 dark:text-zinc-400 animate-pulse">Initializing Admin Interface...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-xl p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">Manage the master problem set.</p>
        </div>
        <button
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-900">
                <th className="py-3 px-4 text-xs font-mono font-semibold text-zinc-500 uppercase">ID</th>
                <th className="py-3 px-4 text-xs font-mono font-semibold text-zinc-500 uppercase">Title</th>
                <th className="py-3 px-4 text-xs font-mono font-semibold text-zinc-500 uppercase">Difficulty</th>
                <th className="py-3 px-4 text-xs font-mono font-semibold text-zinc-500 uppercase text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-150 dark:divide-zinc-900/60">
              {problems.map((prob) => (
                <tr key={prob.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/30 group">
                  <td className="py-3 px-4 font-mono text-sm text-zinc-500">{prob.id}</td>
                  <td className="py-3 px-4 text-sm font-medium text-zinc-900 dark:text-zinc-100">{prob.title}</td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => handleOpenModal(prob)} className="p-1.5 text-zinc-500 hover:text-blue-600 rounded">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(prob.id)} className="p-1.5 text-zinc-500 hover:text-rose-600 rounded">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900/50">
              ...
              <button onClick={handleCloseModal} className="text-zinc-500">✕</button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              {activeModalTab === 'details' ? (
                <form id="problemForm" onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1.5">Problem Title</label>
                    <input
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1.5">Tags</label>
                      <input
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1.5">Description (Markdown)</label>
                    <textarea
                      required value={formData.descriptionMd}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <h4 className="font-semibold text-zinc-900 dark:text-zinc-100">Existing Test Cases</h4>
                    {(!editingProblem?.testCases || editingProblem.testCases.length === 0) ? (
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {editingProblem.testCases.map((tc: any, i: number) => (
                          <div key={tc.id} className="border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 relative">
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-sm font-mono font-bold text-purple-600">Test Case #{i + 1}</span>
                              <div className="flex items-center gap-3">
                                {tc.isHidden ? (
                                  <span className="text-xs bg-rose-100 text-rose-700 px-2 py-0.5 rounded font-medium">Hidden</span>
                                ) : (
                                  <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-medium">Sample/Visible</span>
                                )}
                                <button onClick={() => handleDeleteTestCase(tc.id)} className="text-rose-500 hover:text-rose-700">
                                  <Trash2 className="w-4 h-4" />
                                </button>
                              </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                <span className="text-xs font-semibold text-zinc-500">Input</span>
                                <pre className="text-xs bg-zinc-50 dark:bg-zinc-900 p-2 rounded mt-1 border border-zinc-200 dark:border-zinc-800">{tc.inputData}</pre>
                              </div>
                              <div>
                                <span className="text-xs font-semibold text-zinc-500">Expected Output</span>
                                <pre className="text-xs bg-zinc-50 dark:bg-zinc-900 p-2 rounded mt-1 border border-zinc-200 dark:border-zinc-800">{tc.expectedOutput}</pre>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  <form onSubmit={handleAddTestCase} className="bg-zinc-50 dark:bg-zinc-900/50 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800">
                    <h4 className="font-semibold text-zinc-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
                      <Plus className="w-4 h-4" /> Add Test Case
                    </h4>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Input Data</label>
                        <textarea
                          required value={tcInput} onChange={(e) => setTcInput(e.target.value)}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Expected Output</label>
                        <textarea
                          required value={tcOutput} onChange={(e) => setTcOutput(e.target.value)}
                          className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-lg dark:text-white font-mono text-sm"
                          rows={3}
                        />
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox" checked={tcIsHidden} onChange={(e) => setTcIsHidden(e.target.checked)}
                          className="w-4 h-4 text-purple-600 rounded border-gray-300 focus:ring-purple-500"
                        />
                        <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Is Hidden (Used for grading, not shown to user)</span>
                      </label>
                      <button
                        type="submit" disabled={tcSaving}
            ...
            <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex justify-end gap-3">
              <button onClick={handleCloseModal} className="px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-200 rounded-lg">
                Close
              </button>
              {activeModalTab === 'details' && (
                <button
                  type="submit" form="problemForm" disabled={saving}
                ...
                >
                  {saving ? "Saving..." : editingProblem ? "Save Changes" : "Save & Continue to Test Cases"}
                </button>
              )}
            </div>
